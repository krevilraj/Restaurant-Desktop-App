...
import com.test.client.jframes.KitchenPrinter;
...
public class EmployeeProcessOrders extends JPanel {
	...
	public EmployeeProcessOrders() {
		...
		label_5.setIcon(new ImageIcon(EmployeeProcessOrders.class.getResource("/image/calculator.png")));
		...
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (order_id != -1) {
					if (calculateGrandTotal()) {
						new KitchenPrinter().displaybyId(order_id,txt_subtotal.getText(),txt_discount.getText(),txt_gst.getText(),txt_grandtotal.getText());
					}
				} else {
					JOptionPane.showMessageDialog(null, "First choose order from the list.");
				}
...
//		panel_1.setOpaque(false);
		...
		lblNewLabel.setIcon(new ImageIcon(EmployeeProcessOrders.class.getResource("/image/processorder.jpg")));
...
//		lblNewLabel.setIcon(new ImageIcon(ProcessOrders.class.getResource("/image/bg.jpg")));
		...
		txt_grandtotal.setVisible(false);
	}
